        ...
        switch (block.type) {
          case "heading2":
            return (
              <h2
                key={i}
                className="text-xl font-black text-gray-800 pt-2 border-l-4 border-orange-400 pl-3"
              >
                {block.text}
              </h2>
            );
            ...
            return (
              <ul key={i} className="space-y-3">
                {block.items.map((item, j) => (
                  ...
                  >
                    <span className="text-orange-400 font-black mt-1 flex-shrink-0">✓</span>
          ...
          case "ranking_cta": {
            ...
            return (
              <div
                key={i}
                className={`rounded-2xl p-5 border ${isTop ? "bg-orange-50 border-orange-200" : "bg-gray-50 border-gray-200"}`}
              >
              ...
              <ol key={i} className="space-y-4">
                {block.items.map((step, j) => (
                  <li key={j} className="flex gap-4">
                    <div className="flex-shrink-0 w-8 h-8 bg-orange-400 text-white rounded-full flex items-center justify-center font-black text-sm mt-0.5">
                      {j + 1}
                    </div>
                ...
                <ul className="space-y-2">
                  {block.items.map((item, j) => (
                    <li key={j}>
                      <Link
                        href={`/blog/${item.slug}`}
                        className="flex items-center gap-2 text-base text-orange-500 font-bold hover:text-orange-600 hover:underline underline-offset-2"
                      >
