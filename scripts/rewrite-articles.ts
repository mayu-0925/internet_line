...
【CVR向上のための必須ルール（最重要）】
- 速度が遅い・つながらない・料金が高いなど「困りごと系」のテーマの場合、問題の根本解決として光回線の乗り換えを自然に提案するセクションを必ず含める
- 「今の回線を設定で改善する」で終わらせず、「根本解決は回線自体を変えること」という方向へ誘導する
- 乗り換え提案は押しつけがましくなく、読者の悩みに寄り添う形で「検討の価値がある」と伝える
- 比較・検討系テーマでは、NURO光が最優位である理由を具体的数値で示し、読者の決断を後押しする
- ranking_ctaブロックの直前には必ず1〜2文のブリッジ文（「このような方には〜がおすすめです」など）を paragraph で挿入する
